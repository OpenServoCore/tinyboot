...
pub fn is_boot_requested() -> bool {
    ...
    #[cfg(boot_req_ram)]
    return unsafe { core::ptr::read_volatile(&raw const __boot_request) == BOOT_REQUEST_MAGIC };
}
...
fn drive_boot_pin(pin: crate::Pin, active_high: bool, system_flash: bool) {
    if active_high == system_flash {
        crate::gpio::set_high(pin);
    } else {
        crate::gpio::set_low(pin);
    }
}
